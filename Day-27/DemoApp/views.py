...
def employee(request):
	...
	if request.method == "POST":
		g = EmForm(request.POST)
		if g.is_valid():
			g.save()
			return redirect('/')
	g = EmForm()
	return render(request,'firstapp/employeelist.html',{'k':g,'e':d})

def empupdate(request,h):
	t = Employee.objects.get(id=h)
	if request.method == "POST":
		z = EmForm(request.POST,instance=t)
		if z.is_valid():
			z.save()
			return redirect('/')
	z = EmForm(instance=t)
	return render(request,'firstapp/employeeupdate.html',{'s':z})